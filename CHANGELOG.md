...
- `/index` endpoint. It returns everything from `/page/0` to `/page/9`.
- New `.env` values:
  - `DB_TABLE_NAME` (ℹ now enforced)
  - `REDIS_EXPIRE_TIME_INDEX`
  - `LIMITER_TIME_WINDOW_REGULAR`
  - `LIMITER_TIME_WINDOW_STRICT`
  - `LIMITER_REQUESTS_PER_WINDOW_REGULAR`
  - `LIMITER_REQUESTS_PER_WINDOW_STRICT`

### Updated

- `handler/settings.go` to reflect some of the new `.env` values.
- `README.md`.
- `.gitignore`.
- Quick guide (`static/index.html`).
